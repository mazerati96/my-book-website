...
// Make login page wait for authSystem (prevents ReferenceError)
(function () {
    // Prevent double-initialization if the file is loaded more than once
    if (typeof window !== 'undefined' && window.authSystem) {
        console.log('🔐 authSystem already initialized — skipping auth.js re-execution');
        return;
    }

    // Ensure firebase is available and initialized via firebase-config.js
